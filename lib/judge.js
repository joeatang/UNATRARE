 ...
 * Each judge evaluates the art image via local LM (LM Studio) using their
 * personality prompt and the shared rubric. Scores are aggregated, majority rules.
 *
 * Approval threshold: 28 / 37.5 (see judges.config.json)
 * Borderline [22–28]: human_review_queue
 * Below 22: rejected
 *
 * Primary:  LM Studio (OpenAI-compatible) at LOCAL_LM_URL  (default: http://localhost:1234)
 *           Model: LOCAL_LM_MODEL (default: google/gemma-3-4b)
 * Fallback: Anthropic API if ANTHROPIC_API_KEY is set and LOCAL_LM_URL is not configured
 */
...
// ── Call local LM (LM Studio) or Anthropic fallback ─────────────
async function callJudge(judge, prompt, imageUrl) {
  const useLmStudio = process.env.LOCAL_LM_URL || !process.env.ANTHROPIC_API_KEY;

  if (useLmStudio) {
    return callJudgeLmStudio(judge, prompt, imageUrl);
  }
  return callJudgeAnthropic(judge, prompt, imageUrl);
}
...
export async function judgeToken(tokenName) {
  ...
  const judgeResults = await Promise.all(
    config.judges.map(async (judge) => {
      ...
      try {
        const result = await callJudge(judge, prompt, imageUrl);
        return { judge, result };
      } catch (err) {
