...
async function resolveImageUrl(artUrl) {
  // External URL → fetch server-side to handle redirects and convert to base64
  if (artUrl.startsWith('http')) {
    try {
      ...
      console.warn('[judge] Could not fetch external image:', err.message);
      // Don't fall back to the URL — Groq will reject 3xx redirects.
      // Throwing here lets the judge fail cleanly rather than produce a garbage score.
      throw new Error(`Image fetch failed: ${err.message}`);
    }
  }

  // Local /uploads/TOKENNAME.ext → read from [project root]/public/uploads/
...
async function callJudgeGroq(judge, prompt, imageUrl, config) {
  ...
  if (!res.ok) {
    ...
    const errMsg = `Groq error ${res.status}: ${err.slice(0, 200)}`;
    // Animated GIFs not supported — retry text-only so judges can still score
    if (err.includes('animated GIFs') || err.includes('animated_gif')) {
      console.warn(`[judge] ${judge.id}: animated GIF not supported by Groq — retrying text-only`);
      return callJudgeGroqTextOnly(judge, prompt, config);
    }
...
// Groq text-only fallback — used when image format is unsupported (e.g. animated GIF)
async function callJudgeGroqTextOnly(judge, prompt, config) {
  const model = process.env.GROQ_MODEL || 'meta-llama/llama-4-scout-17b-16e-instruct';
  const textPrompt = `${prompt}\n\n[Note: The art is an animated GIF. Evaluate based on token name, artist description, cultural context, and the Rare Pepe lineage. Score conservatively on CRAFT since you cannot see the animation frames, but do score all five criteria.]`;
  ...
  if (!res.ok) {
    const err = await res.text();
    throw new Error(`Groq text-only error ${res.status}: ${err.slice(0, 200)}`);
  }

  const data = await res.json();
  const text = data.choices?.[0]?.message?.content?.trim();
  if (!text) throw new Error('Empty response from Groq (text-only)');
  return parseJudgeResponse(text, config);
}
