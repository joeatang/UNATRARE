...
function aggregate(results, config) {
  ...
  // ── Gate: hard rules only ─────────────────────────────────────
  // If any judge detected a hard-rule violation (score=1 on sentinel criteria),
  // the token is rejected regardless of average score.
  // Otherwise: approved. Judge opinions are drama, not the verdict.
  let status;
  if (hardRuleTrigger) {
    status = 'rejected';
  } else {
    // Everything that clears hard rules gets in — this matches the original
    // Rare Pepe Scientist philosophy: don't reject on subjective opinion.
    status = 'approved';
  }

  return { status, normalised, hardRuleTrigger, totalJudges: results.length, details };
}
...
export async function judgeToken(tokenName, options = {}) {
  ...
  const { status, normalised, hardRuleTrigger, totalJudges, details } = aggregate(judgeResults, config);
  ...
  if (options?.force) {
    ...
    const verdictPayload = { token_name: tokenName, status, score: normalised, hardRuleTrigger, totalJudges, details, forced: true };
