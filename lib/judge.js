...
function aggregate(results, config) {
  ...
  // Tier 2 — passes hard rules + scores above threshold → approved WITH council stamp
  // Tier 3 — passes hard rules + scores below threshold → approved, no stamp
  // This matches original Rare Pepe Scientist ethos: subjective opinion is commentary,
  // not a gate. The stamp is a co-sign, not a door.
  const threshold = config.scoring.approval_threshold;
  let status;
  let councilCertified;
  if (hardRuleTrigger) {
    status = 'rejected';
    councilCertified = false;
  } else {
    status = 'approved';
    councilCertified = normalised >= threshold;
  }
