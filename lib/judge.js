...
import { ART_SERIES_CAP, LAST_ART_SERIES } from './seriesConfig.js';

// ── Brain modules — richer voice/lore than judges.config.json personality_prompt
import nakamojo   from './brains/prof_naka_c/index.js';
import rarelooney from './brains/prof_j_looney/index.js';
import dankshawn  from './brains/dank_shawn/index.js';
import catalogus  from './brains/dr_m_catalogus/index.js';
import tg00dman   from './brains/theo_goodman/index.js';
import djpepai    from './brains/dj_pepai/index.js';
import chiguiri   from './brains/chiguiripepe/index.js';
import jfrog      from './brains/j_frog/index.js';
...
// ── Current series (most recent active ART series, or 1) ─────────
function currentSeries(db) {
  // Only art series (>0) count toward rollover; Series 0 is Honorary/invite-only.
  const row = db.prepare(
    "SELECT series, COUNT(*) as n FROM tokens WHERE status = 'approved' AND series > 0 GROUP BY series ORDER BY series DESC LIMIT 1"
  ).get();
  if (!row) return 1;
  // Art series seal at ART_SERIES_CAP (69), then graduate to the next series.
  const next = row.n >= ART_SERIES_CAP ? row.series + 1 : row.series;
  // Never open a series beyond the last art series (total-cap guard).
  return Math.min(next, LAST_ART_SERIES);
}
// ── Find conceptually similar prior submissions ──────────────────────────────
// Surfaces potential derivatives as context for judges.
// Does NOT affect approval — flagged for discussion only.
function findSimilarSubmissions(db, tokenName) {
  const name = tokenName.toUpperCase();
  // Strip common Pepe-token prefixes to isolate the conceptual noun
  const concept = name.replace(/^(UNAT|RARE|PEPE|DRAWNAT|DRAWN|BTC|CRYPTO|BASED|NAT)/, '').trim();
  if (concept.length < 3) return [];
  try {
    return db.prepare(`
      SELECT token_name, artist_handle, status
      FROM tokens
      WHERE token_name != ?
        AND UPPER(token_name) LIKE ?
        AND status IN ('approved', 'pending')
      LIMIT 5
    `).all(tokenName, `%${concept}%`);
  } catch {
    return [];
  }
}
...
function buildJudgePrompt(judge, config, submission) {
  ...
  // System message: WHO this judge is + hard rules + scoring calibration.
  // Use rich roleConfig.systemPrompt from brain file (2x richer than personality_prompt);
  // falls back to judges.config.json personality_prompt if brain not loaded.
  const brainPrompt = BRAIN_MAP[judge.id]?.roleConfig?.systemPrompt || judge.personality_prompt;
  const system = `${config.system_prompt_header}\n\n---\n\n${brainPrompt}`;

  // User message: WHAT to evaluate + calibration examples + output format.
  const isVideoArt = submission.art_mime?.startsWith('video/');
  const videoNote = isVideoArt
    ? `\nArt format: ANIMATED VIDEO (${submission.art_mime?.toUpperCase()}) — you are viewing the artist-provided cover/preview image. The actual submission is an animated video art card. Evaluate the cover as representative of the video's visual direction and overall art quality. Do NOT penalize this submission for being a video — video is a valid UNATRARE art format.`
    : '';

  const user = `RUBRIC — score each category 1 to 10:
${rubric}
...
${submission.description ? `Artist description: ${submission.description.slice(0, 300)}` : ''}${videoNote}
...
${scoreFields}
  ...
  "reasoning": "<1-2 sentences written EXACTLY in your character's voice — use your specific speech patterns, references, and personality. Not a generic art description. Sound unmistakably like YOU and no one else on this council.>"
}`;
  ...
  if (/^https?:/.test(artUrl)) {
    ...
    if (!mime) {
      ...
      mime = { gif:'image/gif', webp:'image/webp', jpg:'image/jpeg', jpeg:'image/jpeg',
               ...
               mp4:'video/mp4', webm:'video/webm', mov:'video/quicktime', m4v:'video/mp4' }[ext] || 'image/png';
    }
  }
...
// Per-judge `backend` ("anthropic" | "groq" | "lmstudio") in judges.config.json wins when its key/URL is set; otherwise falls back to env priority.
async function callJudge(judge, prompt, imageUrl, config) {
  const preferred = judge.backend;
  if (preferred === 'anthropic' && process.env.ANTHROPIC_API_KEY) return callJudgeAnthropic(judge, prompt, imageUrl, config);
  if (preferred === 'groq' && process.env.GROQ_API_KEY) return callJudgeGroq(judge, prompt, imageUrl, config);
  if (preferred === 'lmstudio' && process.env.LOCAL_LM_URL) return callJudgeLmStudio(judge, prompt, imageUrl, config);
...
async function callJudgeGroq(judge, prompt, imageUrl, config) {
  ...
  const userContent = imageUrl
    ? [{ type: 'image_url', image_url: { url: imageUrl } }, { type: 'text', text: prompt.user }]
    : prompt.user;
...
// Philosophy: judge COMMENTARY is entertainment, not the gate.
// Approval is automatic unless a hard-rule violation is detected.
// Hard rules trip when any judge scores a hard-rule criterion at 1.
function aggregate(results, config) {
  ...
  // Hard-rule criteria: if ANY judge scores these at 1 → immediate rejection
  const HARD_RULE_CRITERIA = ['cultural_continuity', 'inevitability', 'craft'];

  const details = [];
  let hardRuleTrigger = null; // first violation found
  ...
  for (const { judge, result } of results) {
    ...
    for (const cat of rubric) {
      ...
      judgeScore += score * cat.weight;

      // Check for hard-rule violation: score of 1 on a sentinel criterion
      if (!hardRuleTrigger && HARD_RULE_CRITERIA.includes(cat.id) && score === 1) {
        hardRuleTrigger = {
  ...
  // Normalise to the config's max_possible (69) scale — displayed as cultural score
  const normalised = maxPossible > 0
    ? (weightedTotal / maxPossible) * config.scoring.max_possible
    : 0;

  // ── Gate: three-tier outcome ──────────────────────────────────
  // Tier 1 — hard rule violation → rejected, not in directory
  // Tier 2 — passes hard rules → approved (admin certifies the stamp manually)
  //
  // Note: the AI judges no longer auto-stamp the council_certified flag.
  // The score still gets recorded so admin can see whether judges thought it
  // was above threshold, but the actual stamp is reserved for human review.
  // This keeps a real "pending certification" window where the Telegram
  // mystery teaser lives before the admin reveals the card.
  let status;
  if (hardRuleTrigger) {
    status = 'rejected';
  } else {
  ...
  const councilCertified = false;

  return { status, councilCertified, normalised, hardRuleTrigger, totalJudges: results.length, details };
}
...
export async function judgeToken(tokenName, options = {}) {
  ...
  // Find conceptually similar submissions for derivative flagging (non-blocking)
  const similarSubmissions = findSimilarSubmissions(db, tokenName);
  const derivativeFlag = similarSubmissions.length > 0
    ? { similar: similarSubmissions.map(s => s.token_name) }
    : null;
  if (derivativeFlag) {
    console.log(`[judge] ${tokenName}: possible derivative — similar tokens: ${derivativeFlag.similar.join(', ')}`);
  }

  // Normalize to JPEG for vision APIs. TEXT_ONLY errors are handled gracefully.
  // For video primary art, use the artist-provided cover image if available.
  let resolvedImageUrl = null;
  let textOnlyNote = '';
  try {
    const effectiveUrl  = token.art_mime?.startsWith('video/') && token.art_cover_url
      ? token.art_cover_url
      : imageUrl;
    const effectiveMime = token.art_mime?.startsWith('video/') && token.art_cover_url
      ? '' // let resolveImageUrl detect MIME from extension
      : token.art_mime;
    resolvedImageUrl = await resolveImageUrl(effectiveUrl, effectiveMime);
  } catch (err) {
  ...
  const judgeResults = await Promise.all(
    config.judges.map(async (judge) => {
      const basePrompt = buildJudgePrompt(judge, config, token, similarSubmissions);
  ...
  const { status, councilCertified, normalised, hardRuleTrigger, totalJudges, details } = aggregate(judgeResults, config);

  const now = Math.floor(Date.now() / 1000);

  const judgeNotesJson = JSON.stringify(details);

  // force mode: full DB update including status + council_certified
  // (rejudge re-evaluates everything cleanly)
  if (options?.force) {
    db.prepare(`
      UPDATE tokens SET status=?, judged_at=?, judge_score=?, judge_notes=?, council_certified=? WHERE token_name=?
    `).run(status, now, normalised, judgeNotesJson, councilCertified ? 1 : 0, tokenName);
    const verdictPayload = { token_name: tokenName, status, score: normalised, councilCertified, hardRuleTrigger, derivativeFlag, totalJudges, details, forced: true };
  ...
  if (status === 'approved') {
    ...
    db.prepare(`
      UPDATE tokens SET
        ...
        council_certified = ?
      WHERE token_name = ?
    `).run(series, cardNumber, now, normalised, judgeNotesJson, councilCertified ? 1 : 0, tokenName);
    ...
    try {
      ...
      const ext = filename.split('.').pop().toLowerCase();
      const mime = token.art_mime || (ext === 'jpg' ? 'image/jpeg' : ext === 'png' ? 'image/png' : ext === 'gif' ? 'image/gif' : 'image/webp');
    ...
    // Rejection reason: hardRuleTrigger if tripped, else council commentary
    const rejectionReason = hardRuleTrigger
      ? `Hard rule violation — ${hardRuleTrigger.criterion} (detected by ${hardRuleTrigger.judge}): ${hardRuleTrigger.reasoning}`
      : details
    ...
    db.prepare(`
      UPDATE tokens SET
        ...
        rejection_reason = ?
      WHERE token_name = ?
    `).run(now, normalised, judgeNotesJson, rejectionReason, tokenName);
  ...
  const verdictPayload = { token_name: tokenName, status, score: normalised, councilCertified, hardRuleTrigger, derivativeFlag, totalJudges, details };
