...
const W = {
  ...
  cosignPerArtist:  20,  // points per distinct verified artist who co-signs you
  cosignCapArtists: 8,   // don't reward beyond this many co-signing artists
};
...
export function scoreParts(raw) {
  ...
  const cosignCount = Number(raw.cosign_count || 0);
  ...
  const cosigns   = Math.min(cosignCount, W.cosignCapArtists) * W.cosignPerArtist;
  const score     = base + breadth + earliness + sustained + founder + cosigns;
...
function gatherRaw(db, wallet) {
  ...
  // Distinct verified artists who have co-signed each wallet (Phase 7).
  const cosigns = db.prepare(`
    SELECT torchbearer_wallet AS w, COUNT(DISTINCT artist_sol_address) AS n
    FROM artist_cosigns
    ${wallet ? 'WHERE torchbearer_wallet = ?' : ''}
    GROUP BY torchbearer_wallet
  `).all(...(wallet ? [wallet] : []));
  const cosignMap = new Map(cosigns.map(r => [r.w, Number(r.n || 0)]));
  ...
  return base.map(r => ({
    ...
    cosign_count: cosignMap.get(r.sol_wallet) || 0,
  }));
}
  ...
  const upsert = db.prepare(`
    INSERT INTO trust_scores
      (sol_wallet, score, base, breadth, earliness, sustained, founder,
       total_burned, artists, active_days, early_salutes, cosigns, cosign_count, computed_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, unixepoch())
    ON CONFLICT(sol_wallet) DO UPDATE SET
      ...
      early_salutes = excluded.early_salutes, cosigns = excluded.cosigns,
      cosign_count = excluded.cosign_count, computed_at = excluded.computed_at
  `);
...
// A salute from a more-trusted backer counts for more toward "momentum".
// The multiplier is gentle and bounded by the tier ceilings in practice:
//   KINDLING (score 0) → 1.0×   ·   STEADY (25) → 1.25×
//   TRUSTED (60) → 1.6×   ·   PILLAR (120) → 2.2×   ·   KEEPER (250) → 3.5×
// So credible conviction rises, but no single wallet can run away with it.
const MOMENTUM_WINDOW_DAYS = 7;   // rolling window that defines "rising"
const TRUSTED_MIN_SCORE    = 60;  // TRUSTED tier — used for the "who's backing" line

/**
 * Per-card momentum, weighted by how trusted each recent backer is.
 * This is the transparent "reading glass" over the raw ledger — it never
 * rewrites totals, it just surfaces what credible people are backing NOW.
 *
 * Unscored wallets (no trust_scores row yet) fall back to a 1× weight via
 * COALESCE, so momentum degrades gracefully to raw recent burn until the
 * scheduled/lazy recompute fills scores in.
 *
 * @returns {Map<string, {weighted:number, raw_window:number, total_24h:number,
 *                         backers:number, trusted_backers:number}>}
 */
export function getCardMomentumBatch(db, cardNames, { windowDays = MOMENTUM_WINDOW_DAYS } = {}) {
  ...
  const rows = db.prepare(`
    SELECT
      ...
      SUM(s.amount_display * (1 + COALESCE(ts.score, 0) / 100.0))        AS weighted,
      SUM(s.amount_display)                                              AS raw_window,
      SUM(CASE WHEN s.burned_at >= ? THEN s.amount_display ELSE 0 END)   AS total_24h,
      COUNT(DISTINCT s.sol_wallet)                                       AS backers,
      COUNT(DISTINCT CASE WHEN COALESCE(ts.score, 0) >= ? THEN s.sol_wallet END) AS trusted_backers
    FROM card_salutes s
    LEFT JOIN trust_scores ts ON ts.sol_wallet = s.sol_wallet
    WHERE s.card_name IN (${placeholders})
      AND s.burned_at >= ?
    GROUP BY s.card_name
  `).all(since24h, TRUSTED_MIN_SCORE, ...cardNames, since);

  const map = new Map();
  for (const r of rows) {
    map.set(r.card_name, {
      weighted:        Number(r.weighted || 0),
      raw_window:      Number(r.raw_window || 0),
      total_24h:       Number(r.total_24h || 0),
      backers:         Number(r.backers || 0),
      trusted_backers: Number(r.trusted_backers || 0),
    });
  }
  return map;
}
