...
/**
 * Posts all newly-generated council drops as a single message.
 * One generation run = one Telegram post, multiple council voices.
...
export async function notifyDropsGenerated(newEntries) {
  ...
  try {
    const sections = newEntries.map(e =>
    ...
    const text = clean([
      ...
      sections.join('\n\n'),
      '',
      `<a href="${BASE_URL}/drops">Full Chamber →</a>`,
    ].join('\n'));

    await sendMessage(text);
  } catch (err) {
    console.warn('[telegram] notifyDropsGenerated failed:', err.message);
  }
}
