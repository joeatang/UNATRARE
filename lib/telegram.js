 ...
 *   notifyCashBurnOpen(burn, imagePath)     — admin CASH BURN ceremony opened
 *   notifyCashBurnClose(burn, imagePath)    — admin CASH BURN ceremony closed
 *
...
// Marketplace — alert the ops channel the instant a piece sells, so the art can
// be delivered promptly. Best-effort: no-op if Telegram isn't configured.
export async function notifySale(o = {}) {
  if (!isConfigured()) return { ok: false, skipped: true };
  const via = o.via === 'dispenser' ? ' · dispenser (auto-delivered)' : '';
  const text = `💰 <b>SOLD — ${clean(o.piece || '?')}</b>${via}\n`
    + `Paid: ${clean(String(o.amount || ''))} ${clean(o.currency || '')}\n`
    + `Deliver to: <code>${clean(o.deliveryAddress || '')}</code>\n`
    + `Artist: ${clean(shortAddr(o.artist || ''))}\n`
    + `Order: ${clean(o.orderId || '')}`;
  try { return await sendMessage(text); } catch { return { ok: false }; }
}
...
// Mystery placeholder for any pre-certification announcement (e.g. salute on
// a token still under council review). Returns null gracefully if the asset
// is missing so we degrade to text-only rather than leaking art.
function getMysteryImage() {
  const filePath = path.join(process.cwd(), 'public', 'mystery-card.png');
  try {
    if (existsSync(filePath)) {
      return { filePath, photoUrl: `${BASE_URL}/mystery-card.png` };
    }
  } catch { /* fall through */ }
  return null;
}
...
export async function notifySalute(token, salute, opts = {}) {
  if (!isConfigured()) return;
  try {
    // Show the real art once the card is REVEALED (art is already public on the
    // card page at that point). Council certification is a separate honor and is
    // no longer required for salutes, so we key the image off revealed_at only.
    const isRevealed = !!token.revealed_at;
    const img    = isRevealed ? getArtImage(token) : getMysteryImage();
    ...
    const cardName = isRevealed ? token.token_name : '???';
    const artistFooter = isRevealed
      ? `art by ${artistLine(token)}${totalLine}`
      : `art still hidden — awaiting council certification${totalLine}`;

    const caption = clean([
      `${headline}  ·  <i>${tier.label}</i>`,
      '',
      `<b>${fmtCash(amount)}</b> $CASH burned for <code>${cardName}</code>`,
      `by <b>${wallet}</b>`,
      artistFooter,
      '',
...
// ── Public: cash burn ceremony opens ────────────────────────────────────────
// `burn` is a cash_burns row plus the renderer-derived helpers passed in by
// the caller (already-formatted amount, tier label, character title, etc.).
// `imagePath` is the ABSOLUTE path to the generated PNG.

export async function notifyCashBurnOpen(burn, imagePath) {
  if (!isConfigured()) return;
  try {
    const ordinal  = burn.ordinal != null ? `#${String(burn.ordinal).padStart(3, '0')}` : '';
    const serial   = burn.serial   || '';
    const amount   = burn.amountDisplay || fmtCash(Number(burn.amount || 0));
    const tier     = (burn.tierLabel || '').toUpperCase();
    const charName = (burn.characterTitle || '').toUpperCase();
    const dedicate = burn.card_name
      ? `for <code>${burn.card_name}</code>`
      : 'for the culture';
    const quote = (burn.quote || '').trim();
    const txSig = (burn.tx_sig || '').trim();
    const signer = (burn.admin_wallet || '').trim();
    const proofLine = txSig
      ? `🔗 <a href="https://solscan.io/tx/${txSig}">on-chain proof</a> · signed by <code>${signer ? truncateWallet(signer) : 'admin'}</code>`
      : '';
    const communityOnly = !!burn.community_only;
    const seedLine = communityOnly
      ? `community ceremony · no admin seed`
      : `<b>${amount}</b> $CASH seeded`;

    const caption = clean([
      `🔥 <b>CASH BURN CEREMONY · OPEN</b>`,
      ordinal && serial ? `ceremony <code>${ordinal}</code> · serial <code>${serial}</code>` :
      ordinal           ? `ceremony <code>${ordinal}</code>` :
      serial            ? `serial <code>${serial}</code>` : '',
      '',
      seedLine,
      !communityOnly && tier ? `tier · <b>${tier}</b>` : '',
      dedicate,
      '',
      quote ? `<i>"${quote}"</i>` : '',
      charName ? `— ${charName}` : '',
      proofLine || '',
      '',
      `🪔 <b>anyone holding $CASH can burn into this ceremony</b> (min 69 $CASH)`,
      `<a href="${BASE_URL}/burns/${burn.id || ''}">Join the burn →</a>`,
    ].join('\n'));

    if (imagePath) {
      const result = await sendPhotoFile(imagePath, caption);
    ...
    console.warn('[telegram] notifyCashBurnOpen failed:', err.message);
    return null;
  }
}

// ── Public: cash burn ceremony closes ──────────────────────────────────────

export async function notifyCashBurnClose(burn, imagePath) {
  if (!isConfigured()) return;
  try {
    const ordinal = burn.ordinal != null ? `#${String(burn.ordinal).padStart(3, '0')}` : '';
    const amount  = burn.amountDisplay || fmtCash(Number(burn.amount || 0));
    const tier    = (burn.tierLabel || '').toUpperCase();
    const top     = Array.isArray(burn.topBurners) ? burn.topBurners.slice(0, 3) : [];
    const seed             = Number(burn.seedAmount || 0);
    const contribTotal     = Number(burn.contribTotal || 0);
    const contributorCount = Number(burn.contributorCount || 0);

    const topLines = top.length
      ? top.map((t, i) => {
          const rank = ['🥇', '🥈', '🥉'][i] || `#${i + 1}`;
          const w    = truncateWallet(t.wallet || t.address || '');
          const a    = fmtCash(Number(t.amount || 0));
          return `${rank} ${w} — <b>${a}</b>`;
        }).join('\n')
      : '';
    const txSig = (burn.tx_sig || '').trim();
    const signer = (burn.admin_wallet || '').trim();
    const proofLine = txSig
      ? `🔗 <a href="https://solscan.io/tx/${txSig}">seed proof</a> · signed by <code>${signer ? truncateWallet(signer) : 'admin'}</code>`
      : '';

    const breakdown = (seed > 0 && contribTotal > 0)
      ? `seed <b>${fmtCash(seed)}</b> · community <b>${fmtCash(contribTotal)}</b>`
      : contribTotal > 0
        ? `community burn · <b>${fmtCash(contribTotal)}</b> $CASH from ${contributorCount} ${contributorCount === 1 ? 'wallet' : 'wallets'}`
        : seed > 0
          ? `seed only · <b>${fmtCash(seed)}</b> $CASH`
          : '';

    const contributorLine = contributorCount > 0
      ? `<b>${contributorCount}</b> ${contributorCount === 1 ? 'wallet joined' : 'wallets joined the burn'}`
      : '';

    const caption = clean([
      `🏁 <b>CASH BURN CLOSED</b>`,
      ordinal ? `ceremony <code>${ordinal}</code>` : '',
      '',
      `<b>${amount}</b> $CASH burned total`,
      breakdown,
      tier ? `tier · <b>${tier}</b>` : '',
      contributorLine,
      topLines ? '\ntop contributors:' : '',
      ...
      proofLine || '',
      '',
      `<a href="${BASE_URL}/burns/${burn.id || ''}">Final ledger →</a>`,
    ].join('\n'));
    ...
    console.warn('[telegram] notifyCashBurnClose failed:', err.message);
    return null;
  }
}
