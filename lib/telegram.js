...
// Tries sendPhoto, falls back to sendMessage if Telegram can't fetch the image.
async function sendPhotoOrText(photoUrl, caption) {
  if (!photoUrl) return sendMessage(caption);
  try {
    return await sendPhoto(photoUrl, caption);
  } catch (err) {
    console.warn('[telegram] sendPhoto failed, falling back to text:', err.message);
...
export async function notifySalute(token, salute, opts = {}) {
  ...
  try {
    ...
    await sendPhotoOrText(imgUrl, caption);
  } catch (err) {
