...
export const FEATURES = {
  ...
  reward_reach:    { env: 'FEATURE_REWARD_REACH',    money: false, label: 'Heralds & Reach — social share tracking + Reach score (status only)' },
  ...
  market_public:   { env: 'FEATURE_MARKET_PUBLIC',   money: false, label: 'Marketplace — reveal the /market nav link (endpoints live regardless; fee rails gated separately by FEE_* env)' },
  market_oversell_lock: { env: 'FEATURE_MARKET_OVERSELL_LOCK', money: false, label: 'Marketplace — oversell/sold-out safety lock (atomic per-unit claim on purchase; refuse quotes & purchases once sold out)' },
  market_my_orders: { env: 'FEATURE_MARKET_MY_ORDERS', money: false, label: 'Marketplace — My Purchases lens + buyer-orders endpoint + glossary (read-only)' },
  market_dispenser_release: { env: 'FEATURE_MARKET_DISPENSER_RELEASE', money: false, label: 'Marketplace — dispenser trustless delivery (detect open Counterparty dispenser; auto-confirm delivery on dispense, no manual authority step)' },
  market_reputation: { env: 'FEATURE_MARKET_REPUTATION', money: false, label: 'Marketplace — artist reputation score from DELIVERED on-chain sales (ungameable; self-deals excluded)' },
  market_notify: { env: 'FEATURE_MARKET_NOTIFY', money: false, label: 'Marketplace — Telegram sale alerts on new orders (via TELEGRAM_BOT_TOKEN/CHAT_ID) so art is delivered promptly' },
  market_offers: { env: 'FEATURE_MARKET_OFFERS', money: false, label: 'Marketplace — Make an Offer (non-custodial; a signed intent the artist can accept → converts to a normal verified purchase; NO funds held)' },
  market_auctions: { env: 'FEATURE_MARKET_AUCTIONS', money: false, label: 'Marketplace — Timed auctions (non-custodial; bids are signed intents, highest at close wins → completes as a normal verified purchase; NO funds held/escrowed)' },
  market_messages: { env: 'FEATURE_MARKET_MESSAGES', money: false, label: 'Marketplace — Buyer↔artist message threads per piece (server-backed v1; a Holepunch P2P transport is the planned upgrade)' },
};
