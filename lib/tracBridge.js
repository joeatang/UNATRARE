 ...
 * so all connected peers receive real-time updates.
 *
 * ARCHITECTURE NOTE:
 * The Intercom peer is a Pear application. It must be running on the same machine
 * as this Next.js server for broadcast to work. On the production server this
 * requires Pear to be installed and the peer started separately.
 *
 * If the peer is NOT running (SC_BRIDGE_TOKEN unset or connection refused),
 * broadcast is silently skipped — verdicts still write to SQLite, and the
 * browser SSE stream (/api/events) covers real-time UI updates independently.
 *
 * TO RUN LOCALLY:
 *   cd /Users/joeatang/UNATRARE/intercom && ./start-peer.sh
 *
 * TO RUN PERSISTENTLY (requires PM2 + Pear on target machine):
 *   pm2 start ecosystem.config.cjs --only unatrare-peer
 */
...
export async function broadcastVerdict(verdict) {
  if (!SC_BRIDGE_TOKEN) {
    // Expected on servers without Pear installed — not an error
    console.log('[tracBridge] SC_BRIDGE_TOKEN not set — Trac broadcast disabled (expected in production without peer)');
    return false;
  }
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      try { ws?.close(); } catch {}
      console.log('[tracBridge] SC-Bridge timeout — peer not running, Trac broadcast skipped');
      resolve(false);
    }, 8000);
    ...
    ws.onerror = (err) => {
      clearTimeout(timeout);
      console.log('[tracBridge] SC-Bridge unreachable — peer not running, Trac broadcast skipped');
      resolve(false);
    };
...
/**
 * broadcastCouncilDrops(entries) — broadcast generated council drops to the
 * unatrare-council sidechannel in a single authenticated WS session.
 *
 * Opens one connection, authenticates once, sends all drops, then closes.
 * Safe to fire-and-forget. Silent if peer is unreachable.
 *
 * @param {Array<{judge_id, judge_name, sigil, text, ts}>} entries
 * @returns {Promise<boolean>} true if all sent, false if peer unreachable
 */
export async function broadcastCouncilDrops(entries) {
  if (!SC_BRIDGE_TOKEN || !entries?.length) return false;

  return new Promise((resolve) => {
    let ws;
    const timeout = setTimeout(() => {
      try { ws?.close(); } catch {}
      console.log('[tracBridge] SC-Bridge timeout — council drops broadcast skipped');
      resolve(false);
    }, 10_000);
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === 'auth_ok' && !authed) {
        authed = true;
        ws.send(JSON.stringify({ type: 'join', channel: COUNCIL_CHANNEL }));
    ...
    ws.onerror = () => {
      clearTimeout(timeout);
      console.log('[tracBridge] SC-Bridge unreachable — council drops broadcast skipped');
      resolve(false);
    };
...
/**
 * storeArt(hash, base64Data, mimeType) — store approved art in the Hyperdrive.
 *
