...
/** Admin verification snapshot — totals + top Heralds + recent clicks. Pure read. */
  ...
  try {
    ...
    const totals = db.prepare(`
      SELECT COUNT(*)                         AS clicks,
             ...
             COUNT(DISTINCT referrer_wallet)   AS heralds
      FROM reach_events
    `).get();
    let conversions = 0;
    try {
      conversions = Number(db.prepare('SELECT COUNT(*) AS n FROM referrals').get()?.n || 0);
    } catch { /* referrals may be empty */ }
    const topHeralds = db.prepare(`
      SELECT sol_wallet, reach, clicks, conversions, cards_shared
      FROM reach_scores ORDER BY reach DESC LIMIT 10
    `).all();
    const recent = db.prepare(`
      SELECT card_name, referrer_wallet, ref_host, created_at
      FROM reach_events ORDER BY created_at DESC LIMIT 12
    `).all();
    return {
      totals: {
        ...
        heralds: Number(totals?.heralds || 0),
        conversions,
      },
      topHeralds,
      recent,
    ...
    return { totals: { clicks: 0, visitors: 0, cards: 0, heralds: 0, conversions: 0 }, topHeralds: [], recent: [] };
  }
}
