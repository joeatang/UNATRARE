...
export async function createListing(body) {
  ...
  // 2) Artist address shape — legacy/p2sh (1…/3…) or native segwit (bc1q…).
  const isLegacy = ADDR_RE.counterparty.test(artistAddress);
  const isSegwit = bip322.SEGWIT_RE.test(artistAddress);
  if (!isLegacy && !isSegwit) return { code: 400, error: 'artistAddress must be a Counterparty (1…/3…) or native segwit (bc1q…) address' };

  // 3) Ownership proof — signature over UNATRARE:LIST:<ASSET>.
  //    Legacy artists sign BIP-137; segwit (bc1q) artists sign BIP-322 (XCP Wallet).
  const challenge = bip137.challengeForList(asset);
  const sig = isSegwit
    ? bip322.verifyBip322(artistAddress, challenge, signature)
    : bip137.verifyRaw(artistAddress, challenge, signature);
