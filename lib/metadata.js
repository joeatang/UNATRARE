...
function buildApprovedJson(token) {
  ...
  return {
    ...
    // `image`: 48×48 icon PNG — this is what Freewallet and other wallets render
    // as the thumbnail in asset lists. Per CIP-25 convention: image = icon,
    // image_large = the actual card art.
    // For video tokens: use the cover PNG (no 48×48 of the cover is generated).
    // Fall back to art_url if no icon exists (SVG, audio, or very old uploads).
