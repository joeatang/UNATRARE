...
export function buildMetadataResponse(token) {
  ...
  // Rejected art stays hidden from wallets.
  if (token.status === 'rejected') {
    return { status: 'rejected' };
  }

  // Not yet revealed, but the artwork is already uploaded → serve it anyway.
  //
  // WHY (critical, learned the hard way): Counterparty explorers/wallets — the
  // xchain.io / tokenscan.io "icon worker" that Freewallet reads — crawl an asset's
  // JSON exactly ONCE, right after the artist broadcasts their on-chain issuance.
  // That crawl consistently lands BEFORE UNATRARE approves/reveals the card. If we
  // return {status:pending} (no image) at that moment, CoinDaddy caches a blank
  // "CP"-logo default forever and NEVER re-crawls — so the thumbnail stays broken
  // even after reveal. Serving the image as soon as `art_hash` exists guarantees
  // that single crawl captures real art. The artist has already published this
  // exact URL on their public Bitcoin asset, so the art is not truly "hidden".
  if (token.art_hash) {
    return buildApprovedJson(token);
  }

  // No artwork uploaded yet (reserved URL / pre-submission) — safe pending default.
  return { status: 'pending' };
}
...
function buildApprovedJson(token) {
  ...
  const artExt = art_mime === 'image/jpeg' ? '.jpg'
               ...
               : art_mime === 'video/mp4'  ? '.mp4'
               : art_mime === 'video/webm' ? '.webm'
               : (art_mime === 'video/quicktime' || art_mime === 'video/x-m4v') ? '.mp4'
               : '.jpg'; // safe default
  ...
  // For video primary art, use the cover image directly (uploaded in the wizard).
  // For image art, derived from art_hash as before.
  const icon_url = art_mime?.startsWith('video/') && art_cover_url
    ? absoluteUrl(art_cover_url)
    : (art_hash
  ...
  // Card thumbnail — 400px-wide JPEG generated at upload time (first frame of GIFs).
  // THIS is what Freewallet and other Counterparty wallets render in their thumbnail
  // slot. A 48x48 icon shows up blank/unofficial; the full art (300 KB–3 MB) times out
  // the wallet's thumbnail fetch. The card JPEG (~40-120 KB) is legible AND always loads.
  // Only raster art (png/jpeg/gif/webp) has one; SVG/audio fall back to the full art URL.
  const THUMBABLE = art_mime === 'image/png'  || art_mime === 'image/jpeg'
                 || art_mime === 'image/gif'  || art_mime === 'image/webp';
  const card_url = art_hash && THUMBABLE
    ? (raw_art_url?.startsWith('/uploads/vault/')
        ? `${SITE_URL}/uploads/vault/${art_hash}_card.jpg`
        : `${SITE_URL}/uploads/${art_hash}_card.jpg`)
    : null;
  ...
  // Icon entry intentionally NOT added to images[] here.
  // `image` (v1 field above) is now set to icon_url directly, which is what
  // Freewallet and other wallets read for the 48×48 thumbnail slot.
  // Adding a duplicate icon entry in images[] causes wallets to see two
  // conflicting icon sources and ignore both.
  ...
  let mediaHtml = '';
  if (video_url) {
    // Normalise Apple MIME aliases → standard HTML5 type browsers understand
    const vMime = (video_mime === 'video/quicktime' || video_mime === 'video/x-m4v') ? 'video/mp4' : (video_mime || 'video/mp4');
    mediaHtml = `<video controls autoplay loop playsinline height="560" width="400"><source type="${vMime}" src="${video_url}"></video>\n`;
  } else if (art_mime?.startsWith('video/')) {
    // Primary art itself is a video — embed it for wallets that render description HTML.
    // Use the direct nginx URL (not the canonical /art/ redirect) for cross-origin playback.
    const vMime = (art_mime === 'video/quicktime' || art_mime === 'video/x-m4v') ? 'video/mp4' : art_mime;
    const directVideoUrl = raw_art_url ? absoluteUrl(raw_art_url) : art_url;
    mediaHtml = `<video controls autoplay loop playsinline height="560" width="400"><source type="${vMime}" src="${directVideoUrl}"></video>\n`;
  } else if (audio_url) {
    const aMime = audio_mime || 'audio/mpeg';
    mediaHtml = `<audio controls><source type="${aMime}" src="${audio_url}"></audio>\n`;
  } else if (art_mime?.startsWith('image/')) {
    // Embed the artwork as an <img> so Counterparty wallets/explorers (Freewallet,
    // tokenscan, xchain) actually render the card. This `class="img-responsive"`
    // pattern is THE Rare Pepe / Counterparty convention — without it the asset shows
    // blank/unofficial even when the `image` field is set. Mirrors the working
    // reference token (HITOKEN) and the vault JSON route.
    mediaHtml = `<br /><img src="${art_url}" class="img-responsive" /><br />`;
  }
  ...
  return {
    ...
    // `image`: the wallet thumbnail. Points at the 400px card JPEG (real art that
    // renders in Freewallet without timing out), NOT the 48x48 icon (which shows blank).
    // image_large = the full-resolution card art.
    // For video tokens: use the cover PNG (no card thumb of the cover is generated).
    // Fall back to art_url if no card thumb exists (SVG, audio, or very old uploads).
    image: art_mime?.startsWith('video/')
      ? (art_cover_url ? absoluteUrl(art_cover_url) : (icon_url || ''))
      : (card_url || art_url || ''),
    // `image_large`: the full-size canonical art URL (400×560 card art).
    // For video tokens: the cover PNG (static representation of the video).
    image_large: art_mime?.startsWith('video/')
      ? (art_cover_url ? absoluteUrl(art_cover_url) : (art_url || ''))
      : (art_url || ''),
    ...
    // video field: prefer secondary video_url, fall back to primary art if it is a video.
    // Explicit video field lets tokenscan/wallets play the video rather than
    // relying on the <video> HTML embedded in description.
    ...(video_url
      ? { video: video_url }
      : art_mime?.startsWith('video/') && raw_art_url
        ? { video: absoluteUrl(raw_art_url) }
        : {}),
