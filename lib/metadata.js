...
function buildApprovedJson(token) {
  ...
  // For video primary art, the icon comes from the cover image (uploaded in the wizard).
  // For image art, derived from art_hash as before.
  const cover_hash = art_mime?.startsWith('video/') && art_cover_url
    ? art_cover_url.replace(/^\/uploads\/(?:vault\/)?/, '').split('.')[0]
    : null;
  const icon_url = cover_hash
    ? `${SITE_URL}/uploads/${cover_hash}_icon.png`
    : (art_hash
